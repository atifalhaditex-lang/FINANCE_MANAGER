...
function normalizeFinanceData(parsed) {
    ...
    return {
        ...
        loans: dedupe(source.loans).map(loan => ({
            ...loan,
...
function loanPaidAmount(loan) {
    return loanPayments(loan).reduce(
...
function loanRemaining(loan) {
    return Math.max(0, numberValue(loan?.amount) - loanPaidAmount(loan));
}

function loanStatus(loan) {
    return loanRemaining(loan) <= 0.009 ? "settled" : "active";
}


function loanSortedPayments(loan) {
    return [...loanPayments(loan)].sort(
        (a,b) => String(a.date || "").localeCompare(String(b.date || ""))
    );
}

function loanLastPaymentDate(loan) {
    const payments = loanSortedPayments(loan);
    return payments.length ? payments[payments.length - 1].date : "";
}

function loanSettledDate(loan) {
    return loanStatus(loan) === "settled" ? loanLastPaymentDate(loan) : "";
}


function isAutoFullReturnPayment(loan, payment) {
    if (payment?.autoFullReturn === true) return true;

    const note = String(payment?.note || "");
    const oldAutoNote =
        note === "Full Udhaar returned" ||
        note === "Full Loan received back";

    return Boolean(
        oldAutoNote &&
        loan?.actualReturnDate &&
        payment?.date === loan.actualReturnDate
    );
}

function loanManualPayments(loan) {
    return loanPayments(loan).filter(
        payment => !isAutoFullReturnPayment(loan, payment)
    );
}

function updateLoanFormLabels() {
    const type = $("loanType")?.value || "given";
    const dateLabel = $("loanDateLabel");
    const personLabel = $("loanPersonLabel");
    const typeHelp = $("loanTypeHelp");
    const actualReturnLabel = $("loanActualReturnDateLabel");
    const actualReturnHelp = $("loanActualReturnDateHelp");

    if (type === "received") {
        if (personLabel) personLabel.textContent = "Udhaar kis se liya?";
        if (dateLabel) dateLabel.textContent = "Date I took the Udhaar";
        if (typeHelp) typeHelp.textContent = "Udhaar Taken: you took money from this person and have to return it.";
        if (actualReturnLabel) actualReturnLabel.textContent = "Date I fully returned the Udhaar";
        if (actualReturnHelp) actualReturnHelp.textContent = "Optional. Fill only if the complete Udhaar has already been returned.";
    } else {
        if (personLabel) personLabel.textContent = "Loan kis ko diya?";
        if (dateLabel) dateLabel.textContent = "Date I gave the Loan";
        if (typeHelp) typeHelp.textContent = "Loan Given: you gave money to this person and they have to return it.";
        if (actualReturnLabel) actualReturnLabel.textContent = "Date I fully received the Loan back";
        if (actualReturnHelp) actualReturnHelp.textContent = "Optional. Fill only if the complete Loan has already been received back.";
    }
}
...
function setDefaultLoanForm() {
    ...
    if ($("loanExpectedReturnDate")) $("loanExpectedReturnDate").value = "";
    if ($("loanActualReturnDate")) $("loanActualReturnDate").value = "";
    updateLoanFormLabels();

}


$("loanType")?.addEventListener("change", updateLoanFormLabels);
...
function openLoanForEdit(id) {
    ...
    $("loanPerson").value = loan.person;
    $("loanAmount").value = loan.amount;
    $("loanDate").value = loan.date;
    $("loanExpectedReturnDate").value = loan.expectedReturnDate || "";
    $("loanActualReturnDate").value = loan.actualReturnDate || loanSettledDate(loan) || "";
...
function addLoan(event) {
    ...
    const expectedReturnDate = $("loanExpectedReturnDate")?.value || "";
    const actualReturnDate = $("loanActualReturnDate")?.value || "";
    ...
    if (!date) {
        showToast("Please select the loan date.");
        $("loanDate")?.focus();
        return;
    }

    if (expectedReturnDate && expectedReturnDate < date) {
        showToast("Expected return date cannot be before the loan date.");
        $("loanExpectedReturnDate")?.focus();
        return;
    }

    if (actualReturnDate && actualReturnDate < date) {
        showToast("Actual return date cannot be before the original Loan/Udhaar date.");
        $("loanActualReturnDate")?.focus();
        return;
    }
    if (actualReturnDate && actualReturnDate > today()) {
        showToast("Actual return date cannot be in the future.");
        $("loanActualReturnDate")?.focus();
        return;
    }
    ...
    if (editingLoanId) {
        ...
        if (loan) {
            // Remove only the auto-generated "full return" entry before recalculating.
            // Real partial/manual repayments remain untouched.
            const manualPayments = loanManualPayments(loan);
            const manuallyPaid = manualPayments.reduce(
                (sum, payment) => sum + numberValue(payment.amount),
                0
            );

            if (amount + 0.009 < manuallyPaid) {
                showToast(
                    `Loan/Udhaar amount cannot be less than recorded repayments (${money(manuallyPaid)}).`
                );
                return;
            }

            loan.payments = manualPayments;
            Object.assign(
                loan,
                {type,person,amount,date,expectedReturnDate,actualReturnDate,note}
            );

            if (actualReturnDate && loanRemaining(loan) > 0.009) {
                loan.payments.push({
                    id: uniqueId("loanpay"),
                    amount: loanRemaining(loan),
                    date: actualReturnDate,
                    note: type === "received"
                        ? "Full Udhaar returned"
                        : "Full Loan received back",
                    autoFullReturn: true,
                    createdAt: new Date().toISOString()
                });
            }

            // If the date was cleared, the auto final-payment is also gone.
            // If the loan is fully covered by manual payments, use the last real payment date.
            if (!actualReturnDate && loanRemaining(loan) <= 0.009) {
                loan.actualReturnDate = loanSettledDate(loan);
            }
        }

        showToast(
            type === "received"
                ? "Udhaar updated successfully."
        ...
        if (actualReturnDate) {
            newLoan.payments.push({
                ...
                amount,
                date: actualReturnDate,
                note: type === "received" ? "Full Udhaar returned" : "Full Loan received back",
                autoFullReturn: true,
                createdAt: new Date().toISOString()
            });
        }
        data.loans.push(newLoan);
        showToast(type === "received" ? "Udhaar added successfully." : "Loan added successfully.");
    }
    editingLoanId = null;
    const title = document.querySelector("#loanModal .modal-header h2"); const submit = document.querySelector("#loanForm button[type=submit]");
...
function renderLoans() {
    ...
    const loanGiven = loans.filter(loan => loan.type === "given");
    const udhaarTaken = loans.filter(loan => loan.type === "received");

    const totalGiven = loanGiven.reduce((sum, loan) => sum + numberValue(loan.amount), 0);
    const totalTaken = udhaarTaken.reduce((sum, loan) => sum + numberValue(loan.amount), 0);
    const toReceive = loanGiven.reduce((sum, loan) => sum + loanRemaining(loan), 0);
    const toRepay = udhaarTaken.reduce((sum, loan) => sum + loanRemaining(loan), 0);

    if ($("loanGiven")) $("loanGiven").textContent = money(totalGiven);
    if ($("loanReceived")) $("loanReceived").textContent = money(totalTaken);
    if ($("loanToReceive")) $("loanToReceive").textContent = money(toReceive);
    if ($("loanToRepay")) $("loanToRepay").textContent = money(toRepay);

    const buildTable = (records, mode) => {
        if (!records.length) {
            return `<div class="empty">${mode === "given" ? "No Loan Given records yet." : "No Udhaar Taken records yet."}</div>`;
        }
        ...
        return `
            <table>
                <thead>
                    <tr>
                        <th>${mode === "given" ? "Date Given" : "Date Taken"}</th>
                        <th>${mode === "given" ? "Given To" : "Taken From"}</th>
                        <th>Original</th>
                        <th>${mode === "given" ? "Received Back" : "Repaid"}</th>
                        <th>Remaining</th>
                        <th>Expected Return</th>
                        <th>${mode === "given" ? "Received Date" : "Returned Date"}</th>
                        <th>Status</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody>
                    ${[...records].sort((a,b)=>dateNumber(b.date)-dateNumber(a.date)).map(loan => {
                        const paid = loanPaidAmount(loan);
                        const remaining = loanRemaining(loan);
                        const settled = loanStatus(loan) === "settled";
                        const settledDate = loan.actualReturnDate || loanSettledDate(loan);
                        const payments = [...loanPayments(loan)]
                        ...
                        return `
                            <tr>
                                ...
                                <td>
                                    ...
                                    ${loan.note ? `<small class="loan-row-note">${escapeHTML(loan.note)}</small>` : ""}
                                </td>
                                <td>${money(loan.amount)}</td>
                                <td class="amount-income">${money(paid)}</td>
                                <td class="${remaining > 0 ? "amount-expense" : "amount-income"}">${money(remaining)}</td>
                                <td>${loan.expectedReturnDate ? formatDate(loan.expectedReturnDate) : "—"}</td>
                                <td>${settledDate ? formatDate(settledDate) : "—"}</td>
                                <td><span class="loan-status ${settled ? "settled" : "active"}">${settled ? "Settled" : "Active"}</span></td>
                                <td>
                                    <div class="loan-actions">
                                        <button class="primary-btn compact-btn"
                                            onclick="openLoanPaymentModal('${loan.id}')"
                                            ${settled ? "disabled" : ""}>
                                            ${settled ? "Settled" : (mode === "given" ? "Receive Back" : "Repay Udhaar")}
                                        </button>
                                        <button class="text-btn report-btn" onclick="printFinanceRecord('loan','${loan.id}')">Report</button>
                            ...
                            <tr class="loan-history-row">
                                <td colspan="9">
                                    <div class="loan-history">
                                        <div class="loan-history-head">
                                            <strong>${mode === "given" ? "Loan Return History" : "Udhaar Repayment History"}</strong>
                                            <span>${payments.length} ${payments.length === 1 ? "payment" : "payments"}</span>
                                        </div>
                                        ${
                                            payments.length
                                                ? payments.map(payment => `
                                                    <div class="loan-payment-row">
                                                        <div>
                                                            ...
                                                            <small>${escapeHTML(payment.note || (mode === "given" ? "Money received back" : "Udhaar repayment"))}</small>
                                                        </div>
                                                        <b>${money(payment.amount)}</b>
                                                        <button class="delete-btn" onclick="deleteLoanPayment('${loan.id}','${payment.id}')">Delete</button>
                                                    </div>
                                                `).join("")
                                                : `<div class="empty compact-empty">No payments recorded yet.</div>`
                                        }
                                    </div>
                                </td>
                            </tr>
    ...
    container.innerHTML = `
        <div class="loan-ledger-section">
            <div class="loan-ledger-title">
                <span>LOAN GIVEN</span>
                <h4>Money I gave to others</h4>
                <p>They have to return this money to me.</p>
            </div>
            <div class="table-wrap">${buildTable(loanGiven, "given")}</div>
        </div>

        <div class="loan-ledger-section">
            <div class="loan-ledger-title">
                <span>UDHAAR TAKEN</span>
                <h4>Money I took from others</h4>
                <p>I have to return this money to them.</p>
            </div>
            <div class="table-wrap">${buildTable(udhaarTaken, "received")}</div>
        </div>
    `;
}
...
function openLoanPaymentModal(loanId) {
    ...
    const remaining = loanRemaining(loan);
    if (remaining <= 0.009) {
        showToast("This loan is already settled.");
        return;
    }

    $("loanPaymentLoanId").value = loanId;
    $("loanPaymentAmount").value = "";
    $("loanPaymentDate").value = today();
    $("loanPaymentNote").value = "";
    $("loanPaymentPerson").textContent = loan.person;
    $("loanPaymentRemaining").textContent = money(remaining);

    const title = $("loanPaymentModalTitle");
    const description = $("loanPaymentModalDescription");
    const submit = $("loanPaymentSubmit");

    const paymentDateLabel = $("loanPaymentDateLabel");

    if (loan.type === "received") {
        if (title) title.textContent = "Repay Udhaar";
        if (description) description.textContent = `You took ${money(loan.amount)} from ${loan.person}. Record how much you returned and on which date.`;
        if (submit) submit.textContent = "Save Udhaar Repayment";
        if (paymentDateLabel) paymentDateLabel.textContent = "Date I returned this amount";
    } else {
        if (title) title.textContent = "Receive Loan Back";
        if (description) description.textContent = `You gave ${money(loan.amount)} to ${loan.person}. Record how much they returned and on which date.`;
        if (submit) submit.textContent = "Save Amount Received";
        if (paymentDateLabel) paymentDateLabel.textContent = "Date they returned this amount";
    }

    openModal("loanPaymentModal");
    requestAnimationFrame(() => $("loanPaymentAmount")?.focus());
}
window.openLoanPaymentModal = openLoanPaymentModal;

function saveLoanPayment(event) {
    event.preventDefault();

    const loanId = $("loanPaymentLoanId").value;
    const loan = data.loans.find(item => item.id === loanId);
    if (!loan) return;

    const amount = numberValue($("loanPaymentAmount").value);
    const date = $("loanPaymentDate").value;
    const note = $("loanPaymentNote").value.trim();
    const remaining = loanRemaining(loan);
    ...
    if (!date) {
        showToast("Select the payment date.");
        $("loanPaymentDate")?.focus();
        return;
    }

    if (date < loan.date) {
        showToast(`Payment date cannot be before the loan date (${formatDate(loan.date)}).`);
        $("loanPaymentDate")?.focus();
        return;
    }

    if (date > today()) {
        showToast("Actual payment date cannot be in the future.");
        $("loanPaymentDate")?.focus();
        return;
    }

    if (amount > remaining + 0.009) {
        showToast(`Payment cannot exceed the remaining balance of ${money(remaining)}.`);
        $("loanPaymentAmount")?.focus();
        return;
    }

    const duplicate = loanPayments(loan).some(payment =>
        numberValue(payment.amount) === amount &&
        payment.date === date &&
        String(payment.note || "") === note
    );

    if (duplicate) {
        showToast("This loan payment is already recorded.");
        return;
    }

    loan.payments = loanPayments(loan);
    loan.payments.push({
        id: uniqueId("loanpay"),
        amount,
        date,
        note,
        autoFullReturn: false,
        createdAt: new Date().toISOString()
    });

    if (loanRemaining(loan) <= 0.009) {
        loan.actualReturnDate = date;
    } else {
        loan.actualReturnDate = "";
    }
    ...
    if (loanRemaining(loan) <= 0.009) {
        showToast(`${loan.person}'s loan is now fully settled.`);
    } else {
        showToast(
            loan.type === "received"
                ? "Repayment recorded successfully."
                : "Loan payment received successfully."
        );
    }
}

$("loanPaymentForm")?.addEventListener("submit", saveLoanPayment);

async function deleteLoanPayment(loanId, paymentId) {
    const loan = data.loans.find(item => item.id === loanId);
    if (!loan) return;

    const payment = loanPayments(loan).find(item => item.id === paymentId);
    if (!payment) return;

    if (!(await confirmAction(
        `Delete the loan payment of ${money(payment.amount)}?`,
        {title:"Delete loan payment", confirmText:"Delete"}
    ))) return;

    loan.payments = loanPayments(loan).filter(item => item.id !== paymentId);
    ...
    saveData();
    updateAll();
    showToast("Loan/Udhaar payment deleted.");
}
...
async function deleteLoan(id) {
    ...
    const loan = data.loans.find(item => item.id === id);
    if (!loan) return;
    const paymentCount = loanPayments(loan).length;
    const message = paymentCount
        ? `Delete this loan and its ${paymentCount} recorded payment${paymentCount === 1 ? "" : "s"}?`
        : "Delete this loan record?";
    if(!(await confirmAction(message,{title:"Delete loan",confirmText:"Delete"}))) return;
...
function printFinanceRecord(type,id){
    ...
    if(type==="installment"){
        ...
        const paid=loanPaidAmount(x);
        const remaining=loanRemaining(x);
        title=`Loan Report — ${escapeHTML(x.person||"Record")}`;
        subtitle=x.type==="received" ? "Udhaar Taken — complete repayment history" : "Loan Given — complete return history";
        const lastPaymentDate=loanLastPaymentDate(x);
        const settledDate=loanSettledDate(x);
        rows=`
          <tr><th>Person</th><td>${escapeHTML(x.person||"-")}</td></tr>
          <tr><th>Type</th><td>${x.type==="received" ? "Udhaar Taken" : "Loan Given"}</td></tr>
          <tr><th>Original Amount</th><td>${money(x.amount)}</td></tr>
          <tr><th>${x.type==="received" ? "Amount Repaid" : "Amount Received Back"}</th><td>${money(paid)}</td></tr>
          <tr><th>Remaining</th><td>${money(remaining)}</td></tr>
          <tr><th>Loan Date</th><td>${formatDate(x.date)}</td></tr>
          <tr><th>Expected Return Date</th><td>${x.expectedReturnDate ? formatDate(x.expectedReturnDate) : "-"}</td></tr>
          <tr><th>Last Payment Date</th><td>${lastPaymentDate ? formatDate(lastPaymentDate) : "-"}</td></tr>
          <tr><th>Settled Date</th><td>${settledDate ? formatDate(settledDate) : "-"}</td></tr>
          <tr><th>Status</th><td>${remaining<=0.009 ? "Settled" : "Active"}</td></tr>
          <tr><th>Note</th><td>${escapeHTML(x.note||"-")}</td></tr>`;
        const pays=[...loanPayments(x)].sort((a,b)=>String(a.date||"").localeCompare(String(b.date||"")));
        history=pays.length
          ? `<h3>Payment History</h3><table><thead><tr><th>Date</th><th>Note</th><th>Amount</th></tr></thead><tbody>${pays.map(p=>`<tr><td>${formatDate(p.date)}</td><td>${escapeHTML(p.note || (x.type==="received" ? "Repayment" : "Payment received"))}</td><td>${money(p.amount)}</td></tr>`).join("")}</tbody></table>`
...
function setAuthenticatedVisibility(isAuthenticated) {
    ...
    document.documentElement.classList.toggle("finance-authenticated", isAuthenticated);
}
// AUTH GATE: never expose dashboard before a verified session
function enforceAuthGate(){
    const signedIn = Boolean(currentUser?.id);
    setAuthenticatedVisibility(signedIn);
    if(!signedIn){
        const app = $("financeApp");
...
// Keep dashboard locked when no session exists.
document.addEventListener("visibilitychange", enforceAuthGate);
