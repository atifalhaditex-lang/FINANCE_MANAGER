...
function openTransactionForEdit(id) {
    ...
    if (transaction.source) {
        const sourceNames={installment:"Installments",committee:"Committee","credit-withdrawal":"Credit Card"};
...
async function deleteCreditCard(id) {
    ...
    if (!(await confirmAction(
        "Delete this credit card and its linked cash-withdrawal records?",
...
function printFinanceRecord(type,id){
    ...
    if(type==="installment"){
        ...
        rows=`
          ...
          <tr><th>Already Paid / Historical</th><td>${money(numberValue(p.alreadyPaidAmount))}</td></tr>
          <tr><th>Total Paid</th><td>${money(numberValue(p.downPayment)+installmentPaid(p))}</td></tr>
...
function setAuthenticatedVisibility(isAuthenticated) {
    const app=$("financeApp"), auth=$("authShell");
    if (app) {
        ...
        if (isAuthenticated) app.removeAttribute("inert"); else app.setAttribute("inert","");
    }
    if (auth) {
        ...
        if (isAuthenticated) auth.setAttribute("inert",""); else auth.removeAttribute("inert");
    }
...
async function initializeSecureFinanceAssistant() {
    ...
    // Verify the session with Supabase before exposing any financial UI.
    const { data: verifiedUserData, error } = await supabaseClient.auth.getUser();
    if (error && error.name !== "AuthSessionMissingError") console.warn("Verified session check failed:", error);
    if (verifiedUserData?.user) await openAuthenticatedApp(verifiedUserData.user);
    else closeAuthenticatedApp();
...
function init() {
    ...
    ["installmentTotalPrice","installmentDownPayment","installmentCount","installmentMarkup"]
