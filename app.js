...
function renderInstallments() {
    ...
    let next = null;
    active.forEach(p => {
        const paidCount = installmentHistoricalCount(p) + (Array.isArray(p.payments) ? p.payments.length : 0);
    ...
    list.innerHTML = plans.map(plan => {
        ...
        const percent = total ? Math.min(100, ((numberValue(plan.downPayment)+historicalPaid+paid)/total)*100) : 0;
        ...
        return `<div class="installment-card">
            ...
            <div class="installment-metrics"><div><span>Total Payable</span><strong>${money(total)}</strong></div><div><span>Paid</span><strong class="amount-income">${money(numberValue(plan.downPayment)+historicalPaid+paid)}</strong></div><div><span>Remaining</span><strong class="amount-expense">${money(remainingBal)}</strong></div><div><span>Payments</span><strong>${paidCount}/${plan.numberOfInstallments}</strong></div></div>
