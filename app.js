...
function updateLoanFormLabels() {
    ...
    if (type === "received") {
        if (dateLabel) dateLabel.textContent = "Date I borrowed the money";
        if (typeHelp) typeHelp.textContent =
            "Use this when you received money from someone and you have to pay it back.";
    } else {
        if (dateLabel) dateLabel.textContent = "Date I gave the money";
        if (typeHelp) typeHelp.textContent =
            "Use this when you gave money to someone and they have to return it to you.";
    }
}
...
function renderLoans() {
    ...
    const totalGiven = loans
        ...
        .reduce((sum, loan) => sum + numberValue(loan.amount), 0);
    ...
    const toReceive = loans
        .filter(loan => loan.type === "given")
        .reduce((sum, loan) => sum + loanRemaining(loan), 0);
    ...
    if (!loans.length) {
        container.innerHTML = `<div class="empty">No loan records yet.</div>`;
        return;
    }

    const list = [...loans].sort(
        ...
        <table>
            ...
            <tbody>
                ${list.map(loan => {
                    ...
                    const paymentLabel = loan.type === "received"
                        ? "Repay"
                        : "Receive Back";

                    const typeLabel = loan.type === "received"
                        ? "I Borrowed"
                        : "I Lent";

                    const historyTitle = loan.type === "received"
                        ? "Repayment History"
                        : "Money Received Back";
                    ...
                    return `
                        <tr>
                            ...
                            <td>
                                <span class="badge ${loan.type === "given" ? "badge-given" : "badge-received"}">
                                    ${typeLabel}
                                </span>
                            </td>
                                            ...
                                            ? payments.map(payment => `
                                                <div class="loan-payment-row">
                                                    <div>
                                                        ...
                                                        <small>${escapeHTML(payment.note || (loan.type === "received" ? "Repayment" : "Money received back"))}</small>
                                                    </div>
...
function openLoanPaymentModal(loanId) {
    ...
    if (loan.type === "received") {
        if (title) title.textContent = "Repay Borrowed Money";
        if (description) description.textContent = `Record the amount and the exact date you paid ${loan.person} back.`;
        if (submit) submit.textContent = "Record Repayment";
...
function printFinanceRecord(type,id){
    ...
    if(type==="installment"){
        ...
        subtitle=x.type==="received" ? "Borrowed money and repayment history" : "Money lent and collection history";
        ...
        rows=`
          ...
          <tr><th>Type</th><td>${x.type==="received" ? "I borrowed money — I have to repay it" : "I lent money — they have to return it"}</td></tr>
