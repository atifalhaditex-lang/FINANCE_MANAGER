...
function updateLoanFormLabels() {
    ...
    if (type === "received") {
        if (dateLabel) dateLabel.textContent = "Date I took this Udhaar";
        if (typeHelp) typeHelp.textContent =
            "Use Udhaar Taken when you received money from someone and you have to repay it.";
    } else {
        if (dateLabel) dateLabel.textContent = "Date I gave this loan";
        if (typeHelp) typeHelp.textContent =
            "Use Loan Given when you gave money to someone and they have to return it to you.";
    }
}
...
function setDefaultLoanForm() {
    ...
    if ($("loanExpectedReturnDate")) {
        $("loanExpectedReturnDate").value = "";
    }
...
function addLoan(event) {
    ...
    if (editingLoanId) {
        ...
        if (loan) {
            const alreadyPaid = loanPaidAmount(loan);
            if (amount + 0.009 < alreadyPaid) {
                showToast(`Loan amount cannot be less than already recorded payments (${money(alreadyPaid)}).`);
                return;
            }
            Object.assign(loan, {type,person,amount,date,expectedReturnDate,note});
        }
...
function renderLoans() {
    ...
    const borrowedLoans = loans.filter(loan => loan.type === "received");

    const totalGiven = givenLoans.reduce((sum, loan) => sum + numberValue(loan.amount), 0);
    const totalReceived = borrowedLoans.reduce((sum, loan) => sum + numberValue(loan.amount), 0);
    const toReceive = givenLoans.reduce((sum, loan) => sum + loanRemaining(loan), 0);
    const toRepay = borrowedLoans.reduce((sum, loan) => sum + loanRemaining(loan), 0);

    if ($("loanGiven")) $("loanGiven").textContent = money(totalGiven);
    if ($("loanReceived")) $("loanReceived").textContent = money(totalReceived);
    if ($("loanToReceive")) $("loanToReceive").textContent = money(toReceive);
    if ($("loanToRepay")) $("loanToRepay").textContent = money(toRepay);

    if (!loans.length) {
        container.innerHTML = `<div class="empty">No loan or Udhaar records yet.</div>`;
        return;
    }

    const renderGroup = (items, groupType) => {
        const isBorrowed = groupType === "received";
        const title = isBorrowed ? "Udhaar Taken Records" : "Loan Given Records";
        const subtitle = isBorrowed
            ? "Money you took from others and still need to repay."
            : "Money you gave to others and expect back.";
        const actionLabel = isBorrowed ? "Repay Udhaar" : "Receive Back";
        const historyTitle = isBorrowed ? "Repayment History" : "Money Received Back";
        const paidLabel = isBorrowed ? "Repaid" : "Returned";
        const dueLabel = isBorrowed ? "Expected Repay Date" : "Expected Return Date";
        const emptyText = isBorrowed
            ? "No Udhaar Taken records yet."
            : "No Loan Given records yet.";

        const list = [...items].sort((a,b) => dateNumber(b.date) - dateNumber(a.date));

        return `
            <div class="loan-group-panel">
                <div class="loan-group-head">
                    <div>
                        <h3>${title}</h3>
                        <p>${subtitle}</p>
                    </div>
                            ...
                            <table>
                                ...
                                <tbody>
                                    ${list.map(loan => {
                                        ...
                                        const lastPaymentDate = loanLastPaymentDate(loan);
                                        const settledDate = loanSettledDate(loan);
                                        ...
                                        return `
                                            <tr>
                                                ...
                                                <td>${lastPaymentDate ? formatDate(lastPaymentDate) : "—"}</td>
                                                <td>${loan.expectedReturnDate ? formatDate(loan.expectedReturnDate) : "—"}</td>
                                                <td>
                                                    <span class="loan-status ${settled ? "settled" : "active"}">
                                                        ${settled
                                                            ? `Settled${settledDate ? ` · ${formatDate(settledDate)}` : ""}`
                                                            : "Active"}
                                                    </span>
                                                </td>
                                                <td>
                                                    <div class="loan-actions">
                                                        <button class="primary-btn compact-btn"
                                                            ...
                                                            ${settled ? "disabled" : ""}>
                                                            ${settled ? "Settled" : actionLabel}
                                                        </button>
                                                        <button class="text-btn report-btn"
                                                            onclick="printFinanceRecord('loan','${loan.id}')">Report</button>
                                                        <button class="text-btn"
                                                            onclick="openLoanForEdit('${loan.id}')">Edit</button>
                                                        <button class="delete-btn"
                                                            onclick="deleteLoan('${loan.id}')">Delete</button>
                                                    </div>
                                                </td>
                                            </tr>
                                                                ...
                                                                ? payments.map(payment => `
                                                                    <div class="loan-payment-row">
                                                                        <div>
                                                                            ...
                                                                            <small>${escapeHTML(payment.note || (isBorrowed ? "Repayment" : "Money received back"))}</small>
                                                                        </div>
                                                                        <b>${money(payment.amount)}</b>
                                                                        <button class="delete-btn"
                                                                            onclick="deleteLoanPayment('${loan.id}','${payment.id}')">
                                                                            Delete
                                                                        </button>
                                                                    </div>
                                                                `).join("")
    ...
    container.innerHTML = `
        ${renderGroup(givenLoans, "given")}
        ${renderGroup(borrowedLoans, "received")}
    `;
}
...
function openLoanPaymentModal(loanId) {
    ...
    if (loan.type === "received") {
        ...
        if (description) description.textContent = `Record the exact amount and date you repaid this Udhaar to ${loan.person}.`;
        if (submit) submit.textContent = "Record Udhaar Repayment";
        if (paymentDateLabel) paymentDateLabel.textContent = "Date I repaid this amount";
    } else {
        if (title) title.textContent = "Receive Money Back";
        if (description) description.textContent = `Record the amount and the exact date ${loan.person} returned money to you.`;
        if (submit) submit.textContent = "Record Money Received";
        if (paymentDateLabel) paymentDateLabel.textContent = "Date I received this amount back";
    }
...
async function deleteLoanPayment(loanId, paymentId) {
    ...
    saveData();
    updateAll();
    showToast("Loan payment deleted.");
}
...
function printFinanceRecord(type,id){
    ...
    if(type==="installment"){
        ...
        subtitle=x.type==="received" ? "Udhaar taken and repayment history" : "Loan given and return history";
        ...
        rows=`
          ...
          <tr><th>Type</th><td>${x.type==="received" ? "Udhaar Taken — I borrowed money and I have to repay it" : "Loan Given — I gave money and it has to be returned to me"}</td></tr>
          <tr><th>Original Amount</th><td>${money(x.amount)}</td></tr>
          <tr><th>${x.type==="received" ? "Amount Repaid" : "Amount Returned"}</th><td>${money(paid)}</td></tr>
