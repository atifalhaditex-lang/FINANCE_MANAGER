...
function bindInstallmentModal() {
    const form = $("installmentForm");
    if (!form || form.dataset.bound === "1") return;
    form.addEventListener("submit", saveInstallmentPlan);
...
function openInstallmentModal() {
    ...
    if (!modal) {
        showToast("Installment form could not be loaded.");
        console.error("installmentModal is missing from index.html");
        return;
    }
    bindInstallmentModal();
    const due = $("installmentDueDate");
    if (due) due.value = new Date().toISOString().slice(0, 10);
    modal.classList.add("show");
    modal.setAttribute("aria-hidden", "false");
    setTimeout(() => $("installmentTitle")?.focus(), 50);
}
