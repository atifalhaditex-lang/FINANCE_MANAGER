...
function addLoan(event) {
    ...
    if (editingLoanId) {
        ...
        if (loan) {
            ...
            Object.assign(loan, {type,person,amount,date,note});
        }
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge ${loan.type === "given" ? "badge-given" : "badge-received"}">
                                    ${loan.type === "given" ? "Given" : "Borrowed"}
                                </span>
                            </td>
                                            ...
                                            ? payments.map(payment => `
                                                <div class="loan-payment-row">
                                                    <div>
                                                        ...
                                                        <small>${escapeHTML(payment.note || (loan.type === "received" ? "Repayment" : "Payment received"))}</small>
                                                    </div>
...
function openLoanPaymentModal(loanId) {
    ...
    if (loan.type === "received") {
        ...
        if (description) description.textContent = `Record money you are paying back to ${loan.person}.`;
        if (submit) submit.textContent = "Record Repayment";
    } else {
        if (title) title.textContent = "Receive Loan Payment";
        if (description) description.textContent = `Record money received back from ${loan.person}.`;
        if (submit) submit.textContent = "Record Payment Received";
    }
...
function printFinanceRecord(type,id){
    ...
    if(type==="installment"){
        ...
        rows=`
          ...
          <tr><th>Type</th><td>${x.type==="received" ? "Borrowed from person" : "Given to person"}</td></tr>
          <tr><th>Original Amount</th><td>${money(x.amount)}</td></tr>
          <tr><th>${x.type==="received" ? "Repaid" : "Received Back"}</th><td>${money(paid)}</td></tr>
          <tr><th>Remaining</th><td>${money(remaining)}</td></tr>
          <tr><th>Status</th><td>${remaining<=0.009 ? "Settled" : "Active"}</td></tr>
          <tr><th>Date</th><td>${formatDate(x.date)}</td></tr>
