...
function normalizeFinanceData(parsed) {
    ...
    return {
        ...
        committees: Array.isArray(source.committees) ? source.committees : [],
        creditCards: Array.isArray(source.creditCards) ? source.creditCards : [],
        cashWithdrawals: Array.isArray(source.cashWithdrawals) ? source.cashWithdrawals : [],
...
function setCloudStatus(text, state = "idle") {
    ...
    if (holder) holder.dataset.state = state;
}
...
async function syncFinanceDataNow() {
    ...
    try {
        ...
        setCloudStatus("Saved privately", "saved");
    } catch (error) {
        console.error("Cloud save failed:", error);
        setCloudStatus("Offline copy saved", "error");
    }
}
...
async function loadAccountFinanceData(user) {
    ...
    setCloudStatus("Loading my data…", "saving");
    ...
    try {
        ...
        let remoteData = normalizeFinanceData(row?.data);
        const userCache = loadLocalUserCache(user.id);

        // One-time migration of the old single-browser data into the first signed-in account.
        let legacyData = null;
        try {
            const legacyRaw = localStorage.getItem(STORAGE_KEY);
            if (legacyRaw) legacyData = normalizeFinanceData(JSON.parse(legacyRaw));
        } catch (error) {
            console.warn("Legacy data could not be read:", error);
        }

        if (isFinanceDataEmpty(remoteData) && userCache && !isFinanceDataEmpty(userCache)) {
            remoteData = userCache;
        } else if (
            isFinanceDataEmpty(remoteData) &&
            legacyData &&
            !isFinanceDataEmpty(legacyData)
        ) {
        ...
        isLoadingAccountData = false;
        updateAll();
        setCloudStatus("Using offline copy", "error");
...
async function openAuthenticatedApp(user) {
    ...
    $("financeApp")?.classList.remove("auth-hidden");
...
function closeAuthenticatedApp() {
    ...
    $("financeApp")?.classList.add("auth-hidden");
    $("authShell")?.classList.remove("auth-hidden");
    document.body.classList.remove("authenticated");
    setCloudStatus("Private cloud", "idle");
}
...
async function handleLogin(event) {
    ...
    try {
        ...
        setAuthMessage(
            error?.message || "Could not sign in. Check email and password.",
            "error"
        );
    } finally {
...
async function handleRegister(event) {
    ...
    if (password.length < 8) {
        setAuthMessage("Use at least 8 characters for the password.", "error");
        return;
    }
...
async function handlePasswordRecovery(event) {
    ...
    if (password.length < 8) {
        setAuthMessage("Use at least 8 characters for the new password.", "error");
        return;
    }
    if (password !== confirmPassword) {
        setAuthMessage("The two passwords do not match.", "error");
...
async function initializeSecureFinanceAssistant() {
    ...
    if (!supabaseClient) {
        setAuthMessage(
            "Secure login service could not load. Please check the internet connection.",
            "error"
        );
        return;
    }

    // Existing Supabase browser session: open the correct user's private data.
    ...
    if (sessionData?.session?.user) {
        await openAuthenticatedApp(sessionData.session.user);
    } else {
        closeAuthenticatedApp();
    }
